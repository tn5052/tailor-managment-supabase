...
class _EditInventoryDesktopDialogState
  ...
  void _initializeControllers() {
    ...
    _fullKandoraPriceController = TextEditingController(
      text: (widget.item['full_kandora_price'] ?? 0.0).toString(),
    );
    _adultKandoraPriceController = TextEditingController(
      text: (widget.item['adult_kandora_price'] ?? 0.0).toString(),
    );

    // Add listeners to auto-calculate selling price per unit for fabrics
    if (isFabric) {
  ...
  void _calculateSellingPricePerUnit() {
    ...
    // Don't clear selling price if no kandora prices for editing
  }
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              // Selling price per yard (can be manually edited or auto-calculated)
              _buildTextField(
                controller: _priceController,
                label: 'Selling Price per Yard',
                hint: 'Enter price or auto-calculate from kandora prices',
                icon: PhosphorIcons.calculator(),
                keyboardType: TextInputType.numberWithOptions(decimal: true),
                validator: (value) {
                  if (value == null || value.trim().isEmpty) {
                    return 'Please enter selling price per yard';
                  }
                  if (double.tryParse(value) == null) {
                    return 'Please enter valid amount';
                  }
